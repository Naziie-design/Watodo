...
(() => {
  ...
  const draw=()=>{
    ...
    for(const p of particles){
      ...
      ctx.beginPath(); ctx.fillStyle=`rgba(0,200,255,${p.alpha})`; ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill();
    }
...
(() => {
  ...
  function animate(){
    t+=0.02; const y=Math.sin(t)*6; const r=1+Math.sin(t/1.6)*0.01;
...
function showPopup(){
  ...
  pb.style.display='flex'; pb.setAttribute('aria-hidden','false');
...
// FB Comments visibility fix
window.addEventListener("load", () => {
  const checkFB = setInterval(() => {
    const fbFrame = document.querySelector(".fb-comments iframe");
    if (fbFrame) {
      fbFrame.style.filter = "invert(1) hue-rotate(180deg)";
      fbFrame.style.borderRadius = "12px";
      fbFrame.style.opacity = "0.95";
      fbFrame.style.boxShadow = "0 0 35px rgba(0,230,255,0.35)";
